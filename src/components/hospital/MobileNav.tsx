  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 border-t bg-white z-50">
      <nav className="flex items-center justify-around">
        {mainNavItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className="flex flex-col items-center gap-1 p-2 text-sm">
              <item.icon className={`w-6 h-6 ${isActive ? "text-green-600" : "text-gray-600"}`} />
        ...
        <Button variant="ghost" size="sm" onClick={toggleShowMore} className="flex flex-col items-center gap-1 p-2">
